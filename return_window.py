...
class ReturnInvoiceFrame(ctk.CTkFrame):
    ...
    def process_return(self):
        ...
        try:
            ...
            # --- FIXED: Added a check for missing customer ---
            cursor.execute("SELECT balance FROM customers WHERE id = ?", (customer_id,))
            balance_row = cursor.fetchone()
            if balance_row is None:
                messagebox.showerror("Data Error", f"Could not find customer with ID {customer_id}. The customer may have been deleted.", parent=self)
                conn.rollback()
                return
